...
$(OSAX_SRC): $(OSAX_PATH)/loader.m $(OSAX_PATH)/payload.m
	$(CC) $(OSAX_PATH)/payload.m -shared -fPIC -O2 -mmacosx-version-min=11.0 -arch arm64e -o $(OSAX_PATH)/payload $(FRAMEWORK_PATH) -framework SkyLight -framework Foundation -framework Carbon
	$(CC) $(OSAX_PATH)/loader.m -O2 -mmacosx-version-min=11.0 -arch arm64e -o $(OSAX_PATH)/loader -framework Cocoa
